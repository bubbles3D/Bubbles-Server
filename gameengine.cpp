...
bool GameEngine::approximateColision(GameObject *s1, GameObject *s2){
  ...
  if(dS1 > 0 || dS2 > 0){
    ...
    if(+ux*wx + uy*wy + uz*wz >= 0){
      //L'objet est au niveau du segment ou apr√®s pos
...
bool GameEngine::colidePlayerFlag(Player *p, Flag *f){
  if(p->getTeam() != NULL && p->getFlag() != f){ //Si ce n'est pas le drapeau que le joueur porte dÈj‡
    if(p->getTeam() == f->getTeam()){ //Drapeau mÍme equipe
      if(f->hasBeenMoved()){  //Si il a ÈtÈ bougÈ
        f->respawn(); //On le remet en place
        Logger::log("DRAPEAU AMI REMIS EN PLACE", Logger::GAME_INFO_1);
        return true;
      } else {
        if(p->getFlag() != NULL) { //Si on porte un drapeau
          p->getTeam()->addTeamPoints(1); //Du coup, point marquÈ, respawn drapeau, etc.
          p->getFlag()->respawn();
          p->setFlag(NULL);
          Logger::log("DRAPEAU ENEMI CAPTURE", Logger::GAME_INFO_1);
          return true;
        }
      }
    } else { //Drapeau equipe opposÈe

      p->setFlag(f); //On le prend :D
      f->setOwner(p);
      Logger::log("DRAPEAU ENEMI PRIS", Logger::GAME_INFO_1);
      return true;
    }
  }
